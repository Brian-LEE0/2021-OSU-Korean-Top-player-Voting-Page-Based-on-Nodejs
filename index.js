...
con.connect((err) => {
    ...
    con.query('CREATE TABLE voter (id int not null primary key)', (err, result) => {
    ...
    vote_info.forEach((i, i_index) => i.forEach((j, j_index) => {
        if (j) {
            temp = String(10 - i_index) + "_" + String(1 + j_index) + " VARCHAR(40)"
            if (j_index == 0) temp += " NOT NULL"
            AddRow('voter', temp)
            temp = String(10 - i_index) + "_" + String(1 + j_index) + "_avatar" + " VARCHAR(400)"
            if (j_index == 0) temp += " NOT NULL"
            AddRow('voter', temp)
            temp = String(10 - i_index) + "_" + String(1 + j_index) + "_id" + " VARCHAR(30)"
            if (j_index == 0) temp += " NOT NULL"
            AddRow('voter', temp)
        }
    }))
...
app.post("/submit", (req, res) => {
    ...
    if (data == 1) {
        res.send("<script>alert('투표에 참여해주셔서 감사합니다!\nThank You for Voting!')location.href='/'</script>")
    } else {
        res.send("<script>alert('SysError!!!\n관리자에게 문의해주십시오!')</script>")
    }

})

function retrieveDB(username) {
    con.query("SELECT * FROM voter WHERE username = '" + username + "'", (err, result) => {
        console.log(err)
...
function submit_list(json) {
    ...
    vote_info.forEach((i, i_index) => i.forEach((j, j_index) => {
        if (j) {
            temp += String(10 - i_index) + "_" + String(1 + j_index) + ", "
            temp += String(10 - i_index) + "_" + String(1 + j_index) + "_avatar, "
            temp += String(10 - i_index) + "_" + String(1 + j_index) + "_id"
            if (i_index != vote_info.length - 1 || j_index != i.length - 1) temp += ", "
        }
    }))
    temp += ") VALUES ("
    ...
    vote_info.forEach((i, i_index) => i.forEach((j, j_index) => {
        if (j) {
            if (json[String(10 - i_index) + "_" + String(1 + j_index)]) {
                temp += "'" + json[String(10 - i_index) + "_" + String(1 + j_index)] + "',"
                temp += "'" + json[String(10 - i_index) + "_" + String(1 + j_index) + "_avatar"] + "',"
                temp += "'" + json[String(10 - i_index) + "_" + String(1 + j_index) + "_id"] + "'"
    ...
    con.query(temp, (err, result) => {
        if (err)
            con.query("DELETE FROM voter WHERE id = '" + json.id + "'", (err, result) => {
